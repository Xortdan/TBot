...
function top_time_all()
{
	...
	$number = 1;
	$data ="[hr]";
	$question = "Select nickname, time_all, online FROM user_stats ORDER BY time_all DESC limit ".$config['function']['top_time_all']['count'];
	$toptime = mysqli_query($connect, $question);	
				...
				$tsAdmin->channelEdit($config['function']['top_time_all']['channel'], array(
