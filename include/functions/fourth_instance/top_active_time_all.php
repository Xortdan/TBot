...
function top_active_time_all()
{
	...
	$number = 1;
	$data ="[hr]";
	$question = "Select nickname, active_all, online FROM user_stats ORDER BY active_all DESC limit ".$config['function']['top_active_time_all']['count'];
	$toptime = mysqli_query($connect, $question);	
				...
				$tsAdmin->channelEdit($config['function']['top_active_time_all']['channel'], array(
