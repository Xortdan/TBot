...
function top_idle_time_week()
{
	...
	$number = 1;
	$data ="[hr]";
	$question = "Select nickname, idle_week, online FROM user_stats ORDER BY idle_week DESC limit ".$config['function']['top_idle_time_week']['count'];
	$toptime = mysqli_query($connect, $question);	
				...
				$tsAdmin->channelEdit($config['function']['top_idle_time_week']['channel'], array(
