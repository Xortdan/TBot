...
function top_active_time_week()
{
	...
	$number = 1;
	$data ="[hr]";
	$question = "Select nickname, active_week, online FROM user_stats ORDER BY active_week DESC limit ".$config['function']['top_active_time_week']['count'];
	$toptime = mysqli_query($connect, $question);	
				...
				$tsAdmin->channelEdit($config['function']['top_active_time_week']['channel'], array(
