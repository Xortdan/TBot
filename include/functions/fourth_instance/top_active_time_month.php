<?php 
function top_active_time_month()
{
	global $config;
	global $tsAdmin;
	global $connect;
	...
	$number = 1;
	$data ="[hr]";
	$question = "Select nickname, active_month, online FROM user_stats ORDER BY active_month DESC limit ".$config['function']['top_active_time_month']['count'];
	$toptime = mysqli_query($connect, $question);	
				...
				$tsAdmin->channelEdit($config['function']['top_active_time_month']['channel'], array(
