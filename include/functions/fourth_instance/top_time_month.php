...
function top_time_month()
{
	...
	$number = 1;
	$data ="[hr]";
	$question = "Select nickname, time_month, online FROM user_stats ORDER BY time_month DESC limit ".$config['function']['top_time_month']['count'];
	$toptime = mysqli_query($connect, $question);	
				...
				$tsAdmin->channelEdit($config['function']['top_time_month']['channel'], array(
